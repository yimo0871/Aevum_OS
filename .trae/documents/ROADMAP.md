...
| 方向 | 模块 | 状态 | 关键成果 |
|------|------|------|---------|
| 方向一：检索精度优化 | 混合检索 | ✅ 完成 | 向量+BM25全文检索加权融合 (alpha=0.7) |
| 方向一：检索精度优化 | LLM重排器 | ✅ 完成 | top-K结果LLM评分重排, 优雅降级 |
| 方向一：检索精度优化 | 查询扩展器 | ✅ 完成 | LLM生成语义等价查询变体, 多路召回 |
| 方向一：检索精度优化 | 配置扩展 | ✅ 完成 | config.py 新增7个配置项 |
| 方向一：检索精度优化 | 测试 | ✅ 完成 | 21个新增测试 (hybrid_search 8 + reranker 7 + query_expander 6) |
| 方向三：生产性能优化 | 数据库索引 | ✅ 完成 | 迁移0016 5个复合索引+GIN全文检索 |
| 方向三：生产性能优化 | Redis缓存层 | ✅ 完成 | cache_get/set/invalidate, 无Redis优雅降级 |
| 方向三：生产性能优化 | 批量嵌入 | ✅ 完成 | embed_batch方法, 减少网络往返 |
| 方向三：生产性能优化 | 性能基线脚本 | ✅ 完成 | P50/P95/P99/QPS 压测脚本 |
| 方向三：生产性能优化 | 测试 | ✅ 完成 | 15个新增测试 (cache 12 + embed_batch 3) |

**迭代成果汇总**: 新增36个测试（680 -> 716），数据库迁移 15 -> 16。
...
- 6 个里程碑，16 个迁移，716 个后端单元测试，64 个前端组件测试
