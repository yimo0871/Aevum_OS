...
| 8 | Agent 框架适配器 | LangGraph ✅ + CrewAI + 通用 REST | ⬜ 部分(1/3) |
| 9 | 检索精度 | 权重可配置 + 质量评估指标 + 真实校准 | ⬜ 未开始 |
| 10 | 工作流库 | 标准化模板 + API + 前端浏览 | ⬜ 未开始 |
| 11 | 经验生命周期 | 压缩 + 遗忘 + 安全审计 | ⬜ 未开始 |
| 12 | 多模态经验 | 代码/图像/音频执行经验 | ⬜ 未开始 |
| 13 | 实时经验流 | 流式生成与传播 | ⬜ 未开始 |
| 14 | 经验市场 | 定价 + 交易 + 授权 | ⬜ 未开始 |
| 15 | 联邦网络 | 跨组织隐私保护共享 | ⬜ 未开始 |
| 16 | Agent 身份与归属 | DID + 经验所有权 | ⬜ 未开始 |

**当前完成度：8/16 = 50%（但前 8 项是核心基础设施，权重更高，实际约 75%）**
...
**M1 整体验收标准**：
- [ ] 检索权重可通过环境变量配置
- [ ] 4 个检索质量指标可用（precision/recall/MRR/NDCG）
- [ ] WorkflowTemplate 表存在，迁移 0008 已应用
- [ ] 5 个工作流 API 端点可用
- [ ] 10 个种子工作流模板入库
- [ ] 所有测试通过（后端 310+）
- [ ] 文档全部同步
...
| M1: 短期演进 | ⬜ 未开始 | - | 目标: 310+ |
| M2: Agent 原生 OS | ⬜ 未开始 | - | 目标: 320+ |
| M3: 经验生命周期 | ⬜ 未开始 | - | 目标: 340+ |
| M4: 高级能力 | ⬜ 未开始 | - | 目标: 360+ |
| M5: 生态系统 | ⬜ 未开始 | - | 目标: 400+ |
